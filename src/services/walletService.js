...
// ── Admins autorisés à modifier les fiches ───────────────────
const ADMIN_IDS = [
...
class WalletService {
    ...
    _load() {
        try {
            ...
            console.warn('[WALLET] Erreur chargement:', e.message);
        }
        return {};
    }
    ...
    _save() {
        try {
            ...
            console.warn('[WALLET] Erreur sauvegarde:', e.message);
        }
    }
    ...
    _key(nameOrPseudo) {
        return nameOrPseudo.toLowerCase().trim();
    }

    // ── Trouver un joueur par nom ou pseudo ───────────────────
    findPlayer(nameOrPseudo) {
        const search = this._key(nameOrPseudo);
        for (const [key, player] of Object.entries(this.wallets)) {
            if (
                this._key(player.nom)    === search ||
                this._key(player.pseudo) === search
            ) {
                return { key, player };
            }
        }
    ...
    createWallet(userId, { nom, pseudo, classe, gems = 0, abyssCoins = 0 }, updatedBy) {
        if (!this.isAdmin(userId)) return { success: false, error: 'NOT_AUTHORIZED' };
    ...
    updateGems(userId, nameOrPseudo, amount, operation, updatedBy) {
        ...
        if (operation === 'add') {
            ...
            player.gems = Math.max(0, player.gems - amt);
        } else {
            player.gems = amt; // set direct
        }
    ...
    updateAbyssCoins(userId, nameOrPseudo, amount, operation, updatedBy) {
        ...
        if (operation === 'add') {
            player.abyssCoins += amt;
        } else if (operation === 'remove') {
            player.abyssCoins = Math.max(0, player.abyssCoins - amt);
        } else {
            player.abyssCoins = amt;
        }
    ...
    // ── Toutes les fiches (mise à jour générale) ─────────────
    getAllWallets() {
        return Object.values(this.wallets);
    }

    // ── Formater une fiche en texte ──────────────────────────
    formatWallet(player, updatedBy = null) {
        return formatWallet(player, updatedBy || player.updatedBy || 'L. Lycoris');
    }
